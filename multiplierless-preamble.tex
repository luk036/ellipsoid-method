% -*- coding: utf-8 -*-
% Preamble for multiplierless-slides.md, included via `--include-in-header` on
% the command line (see multiplierless-beamer.yaml).  It lives in a file because
% pandoc parses `header-includes` values as Markdown, which mangles raw LaTeX
% such as fontspec's `[RawFeature={fallback=...}]`.
%
% Compiled with LuaLaTeX because the deck contains literal emoji; the fallback
% font supplies the emoji glyphs that Latin Modern lacks.
\usepackage{fontspec}
